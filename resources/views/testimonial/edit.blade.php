...
@section('content')
    <div class="bg-white shadow rounded-sm px-6 py-8">
        ...
        @endif
        <form id="postForm" enctype="multipart/form-data" method="post" action="{{route('testimonials.update',$testimonial->id)}}">
            @csrf
            @method('PUT')
            <div>
                <input class="custom-input mt-4" placeholder="Name" value="{{$testimonial->name}}" type="input" name="name"/>
                <input class="custom-input mt-4" placeholder="Designation" value="{{$testimonial->designation}}"  type="input" name="designation"/>
                <input class="custom-input mt-4" placeholder="Company" value="{{$testimonial->company}}" type="input" name="company"/>
                <textarea rows="10" cols="50" class="custom-input mt-4" value="{{$testimonial->message}}" placeholder="Message" name="message">{{$testimonial->message}}</textarea>
                ...
                <div class="w-36 h-36 rounded bg-gray-100 mt-4 shadow-sm border border-gray-200">
                    <img class="w-full h-full object-contain" id="imagePreview"
                         src="{{url("storage/images/".$testimonial->image)}}" alt="select iamge">
                </div>
